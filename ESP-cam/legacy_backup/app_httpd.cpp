...
#if defined(LED_GPIO_NUM)
  p += snprintf(p, end - p, ",\"led_intensity\":%u,\"flash\":%u", led_duty, led_duty > 0 ? 1 : 0);
#else
  p += snprintf(p, end - p, ",\"led_intensity\":%d,\"flash\":%d", -1, 0);
#endif
  *p++ = '}';
  *p++ = 0;
...
static esp_err_t flash_handler(httpd_req_t *req) {
  ...
  if (parse_get(req, &buf) == ESP_OK) {
    if (httpd_query_key_value(buf, "val", val_str, sizeof(val_str)) == ESP_OK) {
      flash_val = atoi(val_str);
    } else if (httpd_query_key_value(buf, "state", val_str, sizeof(val_str)) == ESP_OK) {
      if (!strcasecmp(val_str, "on") || !strcasecmp(val_str, "1") || !strcasecmp(val_str, "true")) {
        flash_val = 1;
      } else {
        flash_val = 0;
      }
...
#if defined(LED_GPIO_NUM)
  if (flash_val >= 0) {
    led_duty = flash_val ? 255 : 0;
    enable_led(led_duty > 0);
  }
  char json[64];
  snprintf(json, sizeof(json), "{\"flash\":%d,\"led_intensity\":%d}", led_duty > 0 ? 1 : 0, led_duty);
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  ...
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "{\"error\":\"LED_GPIO_NUM undefined\"}", HTTPD_RESP_USE_STRLEN);
#endif
}
